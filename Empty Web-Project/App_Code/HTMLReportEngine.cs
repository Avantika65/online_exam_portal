 ...
 * Author		:	Jitendra Parihar									*
 * Email		:	jitendraknp@gmail.com								*
 * Date			:	16 Feb 2011											*
 ****************************************************************************/
namespace HTMLReportEngine
{
	...
	public class Report
	{
		...
		{
			htmlContent.Append("<HTML><HEAD><TITLE>Report - " + reportHeading  + "</TITLE></HEAD>" + newline);
			...
			htmlContent.Append("<TABLE Width='100%' style='FILTER: progid:DXImageTransform.Microsoft.Gradient(gradientType=1,startColorStr=#a9d4ff,endColorStr=#ffffff)' Cellpadding=5><TR><TD align='Center'>");
		...
		private void WriteSections() 
		{
			if(sections.Count == 0) 
			{
				...
				htmlContent.Append("<TABLE Width='100%' class='TableStyle'  cellspacing=0 cellpadding=5 border=1px>" + newline);
		...
		private Hashtable WriteSectionDetail(Section section, string criteria) 
		{
			...
			{
				//Draw DetailHeader
                htmlContent.Append("<TR nowrap>" + newline);
				string cellParams="";
				...
				foreach(DataRow dr in reportSource.Tables[0].Select(criteria))
				{
					...
					foreach(Field field in this.reportFields) 
					{
						...
						cellParams += " ALIGN='" + field.alignment + "' ";
                        if (dr[field.FieldName] == "Average")
                        {
                            htmlContent.Append("  <TD bgcolor='whitesmoke' " + cellParams + " VALIGN='top' class='DetailData'>" + dr[field.FieldName] + "</TD>" + newline);
                        }
                        else
                        {
