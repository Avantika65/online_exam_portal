...
public partial class Setup_Mssql : System.Web.UI.Page
{
    ...
    protected void btnMssqlSetup_Click(object sender, EventArgs e)
    {
        if ((string)Session["Type"] == "ToWeb")
        {
            try
            {
                ...
                if (Install(ConnectionString, ""))
                {
                    ...
                    Setup_Setup.SaveWebConfig(ConnectionString, "System.Data.SqlClient");
        ...
        else
        {
            ...
            try
            {
                ...
                {
                    Setup_Setup.SaveWebConfig(ConnectionString + string.Format("Initial Catalog={0};", txtWebCatalog.Text), "System.Data.SqlClient");
                    Session["Step"] = "Settings";
                    Response.Redirect("Default.aspx?Setup=OK");
                }
            }
            catch (Exception ex)
            {
    ...
    private bool Install(string ConnectionString, string AddDatabase) {
        try {
            ...
            Commands = AddDatabase + (Commands.Replace("{0}", strUrl).Replace("{1}", Session["lang"].ToString()));
